...
function jcal_events(jcal, event_callback, recur_event_callback) {
    for (var event of new ICAL.Component(jcal).getAllSubcomponents('vevent'))
    {
...
function event_duration(event) {
    if(event.getFirstPropertyValue('dtend'))
	{
		return new Date(event.getFirstPropertyValue('dtend').toJSDate() - event.getFirstPropertyValue('dtstart').toJSDate()).getTime();
	} else {
		event.updatePropertyWithValue('dtend', event.getFirstPropertyValue('dtstart'));
		return new Date(event.getFirstPropertyValue('dtstart').toJSDate().getTime()+86400000);
	}
}
